	...
	var CTABuilder = {
		...
		bindEvents: function () {
			...
			$(document).on(
				...
				'#cta-config-form input, #cta-config-form select, #cta-config-form textarea',
				function () {
		...
		buildForm: function (templateName) {
			...
			if (['vertical', 'horizontal', 'banner', 'pulse', 'multi-button', 'led-border'].includes(templateName)) {
				...
				html +=
					'<textarea id="cta-description" name="description" rows="3" placeholder="Descrição do seu CTA (opcional)"></textarea>';
				html += '</div>';
				html += '</div>';
			}
		...
		getTemplateSpecificFields: function (templateName) {
			...
			switch (templateName) {
				...
				case 'multi-button':
					...
					html +=
						'<input type="url" id="cta-image" name="image" placeholder="https://exemplo.com/imagem.jpg">';
					html += '</div>';
					html += '</div>';
					html += '</div>';
					break;
				...
				case 'banner':
					...
					html +=
						'<input type="url" id="cta-background" name="background" placeholder="https://exemplo.com/background.jpg">';
					html += '</div>';
					html += '</div>';
					html += '</div>';
					break;
			...
			if (templateName === 'pulse') {
				...
				html +=
					'<input type="text" id="cta-pulse-text" name="pulse_text" placeholder="AO VIVO" value="AO VIVO">';
				html += '</div>';
				html += '<div class="form-group">';
			...
			if (templateName === 'led-border') {
				...
				html +=
					'<input type="text" id="cta-led-colors" name="led_colors" placeholder="#ff0080,#00ff80,#8000ff" value="#ff0080,#00ff80,#8000ff,#ff8000">';
				html += '</div>';
				html += '<div class="form-group">';
		...
		renderStaticPreview: function (shortcode, $container) {
			...
			var html =
				'<div class="alvobot-cta-card alvobot-cta-' +
				template +
			...
			switch (template) {
				case 'vertical':
					...
					if (data.image)
						html +=
							...
							data.image +
							'" style="max-width: 100%; height: auto; border-radius: 6px; max-height: 200px; object-fit: cover;" /></div>';
					if (data.title)
						html +=
					...
					if (data.description)
						html += '<div style="margin: 0 0 20px 0; color: #555;">' + data.description + '</div>';
					if (data.button)
						html +=
							'<a href="#" style="display: inline-block; padding: 12px 24px; background: ' +
							(data.color_button || '#2271b1') +
				...
				case 'horizontal':
					html +=
						'<div style="display: flex; align-items: center; padding: 20px; gap: 20px; flex-wrap: wrap;">';
					if (data.image)
						html +=
							'<div style="flex: 0 0 150px; min-width: 150px;"><img src="' +
							data.image +
							'" style="width: 100%; height: 120px; object-fit: cover; border-radius: 6px;" /></div>';
					...
					if (data.title)
						html +=
							...
							'</h3>';
					if (data.description)
						html += '<div style="margin: 0 0 15px 0; color: #555;">' + data.description + '</div>';
					if (data.button)
						html +=
							'<a href="#" style="display: inline-block; padding: 10px 20px; background: ' +
							(data.color_button || '#2271b1') +
							'; color: white; text-decoration: none; border-radius: 6px;">' +
							data.button +
							'</a>';
					...
					html +=
						'<style>@media (max-width: 480px) { .alvobot-cta-horizontal > div { flex-direction: column !important; text-align: center !important; } .alvobot-cta-horizontal img { max-width: 100% !important; } }</style>';
					break;
				...
				case 'banner': {
					...
					if (data.button)
						html +=
							'<a href="#" style="display: inline-block; padding: 15px 30px; background: ' +
							(data.color_button || '#2271b1') +
							'; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; text-transform: uppercase;">' +
							data.button +
							'</a>';
					html += '</div></div>';
					break;
				...
				case 'simple':
					...
					if (data.title)
						html +=
							'<span style="flex: 1; font-weight: 600; color: ' +
							(data.color_text || '#333') +
							'">' +
							data.title +
							'</span>';
				...
				case 'pulse':
					...
					if (data.subtitle) html += '<p style="margin: 0 0 15px 0; color: #666;">' + data.subtitle + '</p>';
					if (data.description)
						html += '<div style="margin: 0 0 25px 0; color: #555;">' + data.description + '</div>';
					if (data.button)
						html +=
							'<a href="#" style="display: inline-block; padding: 12px 24px; background: ' +
							(data.color_button || '#2271b1') +
				...
				case 'multi-button':
					...
					if (data.button)
						html +=
							'<a href="#" style="padding: 14px 28px; background: ' +
							(data.color_button || '#2271b1') +
					...
					if (data.button2)
						html +=
							'<a href="#" style="padding: 12px 24px; background: ' +
							(data.color_button2 || '#28a745') +
							'; color: white; text-decoration: none; border-radius: 6px; font-weight: 600;">' +
							data.button2 +
							'</a>';
					if (data.button3)
						html +=
							'<a href="#" style="padding: 10px 20px; background: transparent; border: 2px solid ' +
							(data.color_button3 || '#6c757d') +
							'; color: ' +
							(data.color_button3 || '#6c757d') +
				...
				case 'led-border':
					...
					if (data.icon) {
						...
						html +=
							'<div style="margin-bottom: 20px; font-size: 48px; filter: drop-shadow(0 0 10px rgba(255, 255, 255, 0.8));">' +
							iconDisplay +
							'</div>';
					}
					if (data.title)
					...
					if (data.button) {
						...
						html +=
							'<div class="led-border-preview" style="position: relative; display: inline-block; padding: 4px; border-radius: 10px; background: linear-gradient(45deg, #ff0080, #00ff80, #8000ff, #ff8000); background-size: 300% 300%; animation: led-rotate ' +
							ledSpeed +
							' linear infinite;">';
						html +=
							'<a href="#" style="position: relative; display: block; padding: 14px 28px; background: #000; color: #fff; text-decoration: none; border-radius: 6px; font-weight: 700;">' +
							data.button +
							'</a>';
						html += '</div></div>';
					}
				...
				default:
					...
					html +=
						'<p style="color: #666; margin: 0;"><em>Preview estático para template: ' +
						template +
						'</em></p>';
					if (data.title)
						html +=
							'<h3 style="margin: 10px 0; color: ' +
							(data.color_text || '#333') +
							'">' +
							data.title +
							'</h3>';
			...
			animationCSS +=
				'@keyframes led-rotate { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }';
		...
		resetForm: function () {
			...
			$('#cta-preview-content').html(
				'<div class="preview-placeholder"><i data-lucide="eye" class="alvobot-icon"></i><p>Escolha um template para ver o preview</p></div>',
			);
			$('#generated-shortcode').val('');
