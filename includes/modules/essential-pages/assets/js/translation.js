jQuery(document).ready(function ($) {
	...
	function updateSelectedCount() {
		...
		const canTranslate = selectedLanguages > 0 && ($('#translation-mode').val() === 'new' || selectedPages > 0);
	...
	function startTranslation() {
		...
		const actionName =
			mode === 'existing' ? 'alvobot_translate_essential_pages' : 'alvobot_create_translated_pages';
	...
	$(document).on(
		'change',
		'.alvobot-languages-grid input[type="checkbox"], #existing-pages-list input[type="checkbox"]',
		function () {
			updateSelectedCount();
		},
	);
