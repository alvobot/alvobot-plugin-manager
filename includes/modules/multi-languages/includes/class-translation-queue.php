...
class AlvoBotPro_Translation_Queue {
	...
	public function process_next_item() {
		...
		$this->log_progress(
			'system',
			'info',
			'[PROGRESS]Iniciando verificação da fila de tradução',
			[
		...
		if ( $orphaned_count > 0 ) {
			$this->log_progress(
				'system',
				'warning',
				"[FIX]{$orphaned_count} itens órfãos resetados",
				[
		...
		$this->log_progress(
			'database',
			'info',
			'[SEARCH]Buscando e bloqueando próximo item (atomic)',
			[
		...
		if ( $affected_rows === 0 ) {
			$this->log_progress(
				'system',
				'info',
				'[OK]Nenhum item pendente na fila',
				[
		...
		try {
			...
			$this->log_progress(
				'languages',
				'info',
				'[LANG] Processando idiomas de destino',
				[
					'queue_id' => $item->id,
				]
			);
			...
			$this->log_progress(
				'languages',
				'success',
				'[OK]Idiomas de destino decodificados',
				[
					'queue_id'        => $item->id,
					'source_lang'     => $item->source_lang,
		...
		try {
			...
			$logs = [
				[
					'timestamp' => current_time( 'mysql' ),
					'level'     => 'info',
					'message'   => "[START] Iniciado processamento do post '{$post->post_title}'",
				],
			];
				...
				try {
					...
					if ( function_exists( 'pll_get_post_translations' ) ) {
						...
						if ( isset( $translations[ $target_lang ] ) ) {
							$logs[] = [
								'timestamp' => current_time( 'mysql' ),
								'level'     => 'warning',
								'message'   => "[WARNING] Tradução para '{$target_lang}' já existe",
							];
							continue;
						}
					}
					...
					if ( $result['success'] ) {
						...
						$logs[] = [
							'timestamp' => current_time( 'mysql' ),
							'level'     => 'success',
							'message'   => "[OK]Tradução para '{$target_lang}' concluída",
						];
					} else {
						$logs[] = [
							'timestamp' => current_time( 'mysql' ),
							'level'     => 'error',
							'message'   => "[FAIL]Falha ao traduzir para '{$target_lang}': " . ( $result['error'] ?? 'Erro desconhecido' ),
						];
					}
					...
					$logs[] = [
						'timestamp' => current_time( 'mysql' ),
						'level'     => 'error',
						'message'   => "[FAIL]Erro ao traduzir para '{$target_lang}': " . $lang_e->getMessage(),
					];
				}
			}
	...
	private function get_translation_engine() {
		...
		$engine_file = __DIR__ . '/class-translation-engine.php';
	...
	private function translate_article( $post, $source_lang, $target_lang, $translation_engine, &$logs, $queue_id ) {
		try {
			...
			$this->log_progress(
				'article',
				'info',
				'📄 Iniciando tradução de ARTIGO em chunks',
				[
					'queue_id'       => $queue_id,
					'post_id'        => $post->ID,
					'post_title'     => $post->post_title,
			...
			$title_result = $translation_engine->translate_text(
				$post->post_title,
				$source_lang,
				$target_lang,
				[
			...
			if ( ! empty( $excerpt ) ) {
				...
				$excerpt_result = $translation_engine->translate_text(
					$excerpt,
					$source_lang,
					$target_lang,
					[
			...
			$this->log_progress(
				'article',
				'info',
				'[SPLIT]Dividindo conteúdo em chunks',
				[
					'queue_id'            => $queue_id,
			...
			$this->log_progress(
				'article',
				'success',
				'[OK]Conteúdo dividido em chunks',
				[
					'queue_id'       => $queue_id,
			...
			$logs[] = [
				'timestamp'     => current_time( 'mysql' ),
				'level'         => 'info',
				'message'       => '[SPLIT]Conteúdo dividido em ' . count( $content_chunks ) . ' chunks',
				'progress_data' => [
					'total_chunks' => count( $content_chunks ),
			...
			foreach ( $content_chunks as $index => $chunk ) {
				...
				$this->log_progress(
					'chunk',
					'info',
					"[PROGRESS]Traduzindo chunk {$chunk_number}/" . count( $content_chunks ),
					[
						'queue_id'         => $queue_id,
				...
				$logs[] = [
					'timestamp'     => current_time( 'mysql' ),
					'level'         => 'info',
					'message'       => "[PROGRESS]Traduzindo chunk {$chunk_number}/" . count( $content_chunks ),
					'progress_data' => [
						'chunk_index'  => $chunk_number,
						'chunk_length' => $chunk_length,
				...
				$chunk_result = $translation_engine->translate_text(
					$chunk,
					$source_lang,
					$target_lang,
					[
				...
				if ( $chunk_result['success'] ) {
					...
					$this->log_progress(
						'chunk',
						'success',
						"[OK]Chunk {$chunk_number} traduzido com sucesso",
						[
							'queue_id'          => $queue_id,
							'chunk_index'       => $chunk_number,
					...
					$this->log_progress(
						'chunk',
						'error',
						"[FAIL]Falha no chunk {$chunk_number}",
						[
							'queue_id'    => $queue_id,
							'chunk_index' => $chunk_number,
					...
					$logs[] = [
						'timestamp'     => current_time( 'mysql' ),
						'level'         => 'warning',
						'message'       => "[FAIL]Falha ao traduzir chunk {$chunk_number}, mantendo original",
						'progress_data' => [
							'error' => $chunk_result['error'] ?? 'Erro desconhecido',
						],
					];
			...
			$this->log_progress(
				'article',
				'info',
				'[STATS]Resumo da tradução de chunks',
				[
					'queue_id'           => $queue_id,
			...
			foreach ( $translatable_strings as $index => $string_data ) {
				...
				if ( empty( trim( $string_text ) ) ) {
					$translated_strings[ $index ] = [
						'success'         => true,
						'translated_text' => $string_text,
					];
					continue;
				}

				$string_result = $translation_engine->translate_text(
					$string_text,
					$source_lang,
					$target_lang,
					[
	...
	private function split_content_into_chunks( $content, $max_words = 500 ) {
		...
		$content = preg_replace_callback(
			'/\[quiz[^\]]*\](.*?)\[\/quiz\]/s',
			function ( $matches ) use ( &$protected_blocks, &$block_id ) {
				$placeholder                      = "<!--PROTECTED_BLOCK_{$block_id}-->";
				$protected_blocks[ $placeholder ] = $matches[0];
	...
	private function update_progress_realtime( $queue_id, $progress, $status_message = '' ) {
		...
		$this->log_progress(
			'progress',
			'info',
			"[STATS]Progresso atualizado: {$progress}%",
			[
				'queue_id' => $queue_id,
				'progress' => $progress . '%',
	...
	private function parse_openai_log( $log ) {
		...
		// Extrair JSON do log
		if ( strpos( $message, '[START] OPENAI REQUEST:' ) !== false ) {
		...
		if ( strpos( $message, '[RESPONSE] OPENAI:' ) !== false ) {
			$json_start = strpos( $message, '{' );
			if ( $json_start !== false ) {
				$json_data   = substr( $message, $json_start );
				$parsed_data = json_decode( $json_data, true );
		...
		if ( strpos( $message, '[SUCCESS] OPENAI:' ) !== false ) {
			$json_start = strpos( $message, '{' );
			if ( $json_start !== false ) {
				$json_data   = substr( $message, $json_start );
				$parsed_data = json_decode( $json_data, true );
		...
		if ( strpos( $message, '[ERROR] OPENAI API:' ) !== false ) {
			$json_start = strpos( $message, '{' );
			if ( $json_start !== false ) {
				$json_data   = substr( $message, $json_start );
				$parsed_data = json_decode( $json_data, true );
	...
	private function extract_log_level( $message ) {
		...
		if ( strpos( $message, 'WARNING' ) !== false || strpos( $message, '[WARNING]' ) !== false ) {
			return 'warning';
		}
