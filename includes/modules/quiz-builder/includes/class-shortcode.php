...
class Alvobot_Quiz_Shortcode {
	...
	public function handle_early_lead_submission() {
		...
		if ( preg_match( '/\[quiz[^\]]*\](.*?)\[\/quiz\]/s', $content, $matches ) ) {
			...
			} elseif ( method_exists( 'AlvoBotPro', 'debug_log' ) ) {
	...
	private function generate_lead_capture_html( $questions, $quiz_step, $answers, $atts, $quiz_id, $answer_string ) {
		...
		if ( ! empty( $question['fields']['name'] ) ) {
			...
			$output .= '<span class="input-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span>';
		...
		if ( ! empty( $question['fields']['email'] ) ) {
			...
			$output .= '<div class="input-with-icon">';
			$output .= '<span class="input-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg></span>';
...
SCRIPT;
	...
	public function ensure_content_formatting( $content ) {
		...
		$content = preg_replace_callback(
			$quiz_pattern,
			function ( $matches ) use ( &$quiz_blocks, &$index, $placeholder_prefix, $placeholder_suffix ) {
