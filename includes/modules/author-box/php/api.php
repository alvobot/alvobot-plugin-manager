...
class AlvoBotPro_AuthorBox_API {
	...
	public function update_author_info( $request ) {
		...
		$lang_code = isset( $json['lang_code'] ) ? sanitize_key( $json['lang_code'] ) : '';
		if ( $description ) {
			if ( ! empty( $lang_code ) && preg_match( '/^[a-z]{2}$/', $lang_code ) ) {
				update_user_meta( $user->ID, 'ab_bio_' . $lang_code, wp_kses_post( $description ) );
				$response[ 'description_' . $lang_code ] = 'Descrição atualizada para ' . $lang_code;
