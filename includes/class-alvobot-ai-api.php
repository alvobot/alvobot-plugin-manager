...
class AlvoBotPro_AI_API {
	...
	public function call( $action, $params = array() ) {
		$token = get_option( 'alvobot_site_token' );
		if ( empty( $token ) ) {
	...
	public function ajax_generate() {
		...
		$generation_type = isset( $_POST['generation_type'] ) ? sanitize_text_field( wp_unslash( $_POST['generation_type'] ) ) : '';
		$params_raw      = isset( $_POST['params'] ) ? wp_unslash( $_POST['params'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in loop below.
