...
class AlvoBotPro {
	...
	private static $module_registry = array(
		...
		'quiz-builder-v2'      => array(
			...
			'name'          => 'Quiz Builder',
			'description'   => 'Quiz em tela cheia com captura de leads e redirecionamento para artigos.',
			'default'       => true,
...
'smart-internal-links' => array(
		...
		'pixel-tracking'       => array(
			...
			'name'          => 'Pixel Tracking',
			'description'   => 'Rastreamento com Meta Pixel, Google Analytics (GA4) e Google Ads.',
			'default'       => true,
		...
		'missed-schedule'      => array(
			'file'          => 'includes/modules/missed-schedule/class-missed-schedule.php',
			'class'         => 'AlvoBotPro_MissedSchedule',
			'name'          => 'Missed Schedule Publisher',
			'description'   => 'Publica automaticamente posts agendados que perderam o prazo.',
			'default'       => true,
			'settings_slug' => null,
			'admin_menu'    => false,
			'force_active'  => true,
		),
		'mini-preloader'       => array(
			'file'          => 'includes/modules/mini-preloader/class-mini-preloader.php',
			'class'         => 'AlvoBotPro_MiniPreloader',
			'name'          => 'Mini Preloader',
			'description'   => 'Preloader de carregamento com animações, cores e timing personalizáveis.',
			'default'       => false,
			'settings_slug' => 'alvobot-mini-preloader',
			'admin_menu'    => array(
				'page_title' => 'Mini Preloader',
				'menu_title' => 'Mini Preloader',
			),
		),
	);
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_add_inline_style(
			...
			/* AlvoBot connection notice — loads on ALL admin pages */
			. '.notice.alvobot-conn-notice{background:none!important;border:none!important;padding:0!important;margin:5px 0 15px!important;box-shadow:none!important;overflow:hidden;border-radius:8px}'
			. '.alvobot-conn-notice-inner{display:flex;gap:12px;padding:14px 16px;border-left:4px solid;border-radius:8px;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}'
			. '.alvobot-conn-notice-error .alvobot-conn-notice-inner{background:#FEE2E2;border-left-color:#F63D68;color:#344054}'
			. '.alvobot-conn-notice-warning .alvobot-conn-notice-inner{background:#FFFAEB;border-left-color:#FDB022;color:#344054}'
			. '.alvobot-conn-notice-icon{flex-shrink:0;margin-top:1px}'
			. '.alvobot-conn-notice-error .alvobot-conn-notice-icon{color:#F63D68}'
			. '.alvobot-conn-notice-warning .alvobot-conn-notice-icon{color:#FDB022}'
			. '.alvobot-conn-notice-body{flex:1;min-width:0}'
			. '.alvobot-conn-notice-header{display:flex;align-items:center;gap:8px;margin-bottom:5px}'
			. '.alvobot-conn-notice-title{font-weight:600;font-size:14px;margin:0;color:#18181B}'
			. '.alvobot-conn-notice-badge{font-size:11px;padding:2px 8px;border-radius:9999px;font-weight:500;white-space:nowrap}'
			. '.alvobot-conn-notice-error .alvobot-conn-notice-badge{background:rgba(246,61,104,.12);color:#D42F59}'
			. '.alvobot-conn-notice-msg{margin:0 0 8px;font-size:13px;font-weight:500}'
			. '.alvobot-conn-notice-details{margin:0 0 10px;padding:0 0 0 18px;font-size:12px;color:#52525B}'
			. '.alvobot-conn-notice-details li{margin-bottom:3px}'
			. '.alvobot-conn-notice-actions{display:flex;gap:8px;align-items:center;margin-bottom:8px;flex-wrap:wrap}'
			. '.alvobot-conn-btn{display:inline-flex;align-items:center;gap:5px;padding:6px 14px;border-radius:6px;font-size:12px;font-weight:500;text-decoration:none!important;cursor:pointer;border:1.5px solid transparent;transition:all 150ms ease;line-height:1.4;white-space:nowrap}'
			. '.alvobot-conn-btn-primary{background:#F63D68;color:#fff!important;border-color:#F63D68}'
			. '.alvobot-conn-btn-primary:hover{background:#D42F59!important;color:#fff!important;border-color:#D42F59}'
			. '.alvobot-conn-btn-ghost{background:transparent;color:#6B7280!important;border-color:transparent;font-size:12px;padding:6px 4px}'
			. '.alvobot-conn-btn-ghost:hover{color:#18181B!important;text-decoration:underline!important}'
			. '.alvobot-conn-notice-meta{font-size:11px;color:#A1A1AA;display:block;margin-top:2px}'
		);
